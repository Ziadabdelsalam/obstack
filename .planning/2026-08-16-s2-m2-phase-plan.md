...
- **run-goal (verbatim):** obstack M2 sprint 2 — real trace and log search on ClickHouse with persisted saved views (exit: the traces list and LogsExplorer both search real data with real totals, pagination and workspace-persisted saved views).
- **exit criterion:** `/app/traces` filters (service, status, duration, model, cost range, free text over prompts and logs, time range — PRD §8) run server-side with a real total and pagination instead of the 200-cap denominator; `/app/logs` renders from the `logs` table with severity/pod/free-text filters and its real empty state; saved views persist per workspace and survive a reload and a new browser; `/app/logs` is registered in `src/lib/live-routes.ts` and carries no SAMPLE badge in live mode; mock mode unchanged.
