...
- S2.2 — collector distro + nearby log join on kind: **breakdown complete (2026-08-16), plan at `.planning/2026-08-16-s2.2-team-plan.md`** — 5 tasks, 3 waves (W1: collector distro ∥ GenAI event extraction ∥ nearby-join query layer; W2: the real Helm chart per D35; W3: stack-on-kind CI + the D37 evidence bundle + the K5 revisit). Four advisor critical items (collector config, nearby-join SQL, chart structure, D37 evidence). **Awaiting the kickoff gate on three questions**, one of which blocks a task: whether the GenAI event ruling means span events (implementable as "internal/mapping only") or separate log records (which the D7 schema cannot absorb without a writer/schema change, D23 having rejected `ReplacingMergeTree`). **Breakdown complete (2026-08-16), plan at `.planning/2026-08-16-s2.1-team-plan.md`** — 4 tasks, 3 waves; dispatches only after S2.0 merges (its exit needs S2.0's checks to exist).
