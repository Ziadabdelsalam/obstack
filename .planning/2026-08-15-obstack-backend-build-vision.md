...
- S2.1 — monorepo restructure (`apps/web` + npm workspaces; advisor D33, 2026-08-16): planned, cleared for breakdown — purely mechanical one-PR move, guarded by S2.0's CI, before any M2 dev work; restructure PR is an advisor critical item. `packages/obstack-js` is created by S2.4, not here. **Breakdown complete (2026-08-16), plan at `.planning/2026-08-16-s2.1-team-plan.md`** — 4 tasks, 3 waves; dispatches only after S2.0 merges (its exit needs S2.0's checks to exist).
