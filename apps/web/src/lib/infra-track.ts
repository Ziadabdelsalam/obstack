 ...
 * every deployment. The second half is now produced — `server/adapters.ts`
 * fills `k8sEvents` from the rows the collector's `k8s_events` receiver writes,
 * for traces whose spans ran on those pods — but "produced" is not "always
 * there": a workspace not running that receiver, a trace off Kubernetes
 * entirely, or simply a window in which nothing happened all leave the key
 * absent, and it is false in the demo too on any story that carries no events,
 * which includes the pipeline trace the landing page uses as its screenshot. A
 * header naming a data type the timeline below it does not contain is a
 * capability claim, and D208's line is that demo DATA is allowed where a false
